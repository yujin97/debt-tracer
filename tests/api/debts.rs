...
async fn create_debt_returns_a_200_for_valid_json_data() {
    ...
    let response = test_app.post_debt().await;
...
async fn get_debts_returns_a_list_of_debts() {
    ...
    let debt = json_result.unwrap()[0].clone();
    ...
    assert_eq!(debt.amount, 3000.0);
    assert_eq!(debt.currency, "JPY".to_owned());
}
